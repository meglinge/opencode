...
- `openai_responses` - Use OpenAI's Responses API `/responses/compact` endpoint for OpenAI model compaction. The compacted state is encrypted and opaque rather than a readable Markdown summary (default: `true`; set to `false` to disable).
